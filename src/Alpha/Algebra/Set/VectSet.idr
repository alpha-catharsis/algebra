...
export
vectPointedSet : {a : Type} -> {xs : Vect k a} -> SetDec (VectSet xs) ->
                 (x : a) -> {auto prf : VE.Elem x xs} ->
                 PointedSet (VectSet xs)
vectPointedSet sd x = pointedSet sd (nullryFn (VectSet xs) x {prf})
